...
public class PlayerController : MonoBehaviour
{
    ...
    bool withinStealingRange() //bool function to check if the player is near the table (where gold is kept)
    {
        ...
        return Vector3.Distance(gameObject.transform.position, table.position) <= 5f; //vector.dist is used to find the dist between the first parameter and the second
    }
